  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (response.status === 201) {
        showSnackbar("Función creada con éxito", "success");
        navigate("/schedules/list");
      }
    } catch (error) {
      if (error.status === 400) {
        showSnackbar(
          "Error al crear la función, parece que ya tienes una función en esa sala y horario.",
          "error"
        );
      } else {
        showSnackbar(
          "Error al crear la película, por favor, contacta con soporte o intenta nuevamente.F",
          "error"
        );
      }
    }
